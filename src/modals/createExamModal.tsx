...
import {CreateExamRequest, ExamType, examTypeLabels, SubgroupResponse} from "../shared/models";
...
interface CreateExamModalProps {
    ...
    courseId: string;
    onCreate: (data: CreateExamRequest) => void;
}

export default function CreateExamModal({open, setOpenModal, subgroups, courseId, onCreate}: CreateExamModalProps) {
    const [title, setTitle] = useState('');
    const [location, setLocation] = useState('');
    const [subgroup, setSubgroup] = useState<SubgroupResponse | undefined>();
                        ...
                        <DialogPanel className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                <Input
                                    ...
                                    className="bg-[#f9f9f9] text-black"
                                />
                                <Listbox value={subgroup} onChange={setSubgroup}>
                                    <div className="relative">
                                        <ListboxButton
                                            className="bg-[#f9f9f9] text-black flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
                                            {subgroup
                                                ? subgroup.name
                                                : <span className="text-gray-500/60 text-sm">Ենթախումբ</span>}
                                        </ListboxButton>
                                        <Transition
                                            as={Fragment}
                                ...
                                <Listbox value={type} onChange={setType}>
                                    <div className="relative">
                                        <ListboxButton
                                            ...
                                            {type
                                                ? examTypeLabels[type]
                                                : <span className="text-gray-500/60 text-sm">Քննության տեսակ</span>}
                                        </ListboxButton>
                                        <Transition
                                            as={Fragment}
                                            leave="transition ease-in duration-100"
                                            ...
                                            <ListboxOptions
                                                className="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-auto py-1 text-base">
                                                <ListboxOption
                                                    key={ExamType.MIDTERM}
                                                    value={ExamType.MIDTERM}
                                                    className={({focus, selected}) =>
                                                        `text-sm cursor-pointer select-none px-3 py-2 ${focus ? 'bg-[#1B84FF]/20 text-[#1B84FF]' : 'text-gray-900'} ${selected ? 'font-bold' : 'font-normal'}`
                                                    }>
                                                    {examTypeLabels[ExamType.MIDTERM]}
                                                </ListboxOption>
                                                <ListboxOption
                                                    key={ExamType.GENERAL}
                                                    value={ExamType.GENERAL}
                                                    className={({focus, selected}) =>
                                                        `text-sm cursor-pointer select-none px-3 py-2 ${focus ? 'bg-[#1B84FF]/20 text-[#1B84FF]' : 'text-gray-900'} ${selected ? 'font-semibold' : 'font-normal'}`
                                                    }>
                                                    {examTypeLabels[ExamType.GENERAL]}
                                                </ListboxOption>
                                                <ListboxOption
                                                    key={ExamType.REPEAT}
                                                    value={ExamType.REPEAT}
                                                    className={({focus, selected}) =>
                                                        `text-sm cursor-pointer select-none px-3 py-2 ${focus ? 'bg-[#1B84FF]/20 text-[#1B84FF]' : 'text-gray-900'} ${selected ? 'font-semibold' : 'font-normal'}`
                                                    }>
                                                    {examTypeLabels[ExamType.REPEAT]}
                                                </ListboxOption>
                                            </ListboxOptions>
                                        </Transition>
                                    </div>
